...
def add_price_features(df: pd.DataFrame, horizons=(1, 3, 5)) -> pd.DataFrame:
    """
    Add price-side features and future realized-volatility targets.
    ...
    for asset, g in df.groupby("asset", sort=False):
        ...
        for h in horizons:
            future_sq = pd.concat(
                [g["squared_log_return_1d"].shift(-i) for i in range(1, h + 1)],
                axis=1
            )
            ...
            full_return_window = future_sq.notna().sum(axis=1) == h
...
def main():
    ...
    target_cols = [c for c in prices.columns if c.startswith("target_rv")]
