...
def normalize_event_frame(df: pd.DataFrame, source: Path) -> pd.DataFrame:
    ...
    if "date_added_ts_ny" not in df.columns:
        raise ValueError(f"{source} missing date_added_ts_ny")

    df["date_added_ts_ny"] = pd.to_datetime(df["date_added_ts_ny"], errors="coerce")
    if df["date_added_ts_ny"].dt.tz is None:
        df["date_added_ts_ny"] = df["date_added_ts_ny"].dt.tz_localize(
            "America/New_York",
            ambiguous="NaT",
            nonexistent="NaT",
        )
    else:
        df["date_added_ts_ny"] = df["date_added_ts_ny"].dt.tz_convert("America/New_York")

    df = df.dropna(subset=["date_added_ts_ny"])
