...
def build_daily_event_features(event_long, trading_calendar, top_root_k=20, top_country_k=20):
    ...
    root_df = event_long[event_long["EventRootCode"].isin(top_roots)].copy()

    root_pivot = (
        root_df
        .groupby(["date", "EventRootCode"])["n_events"]
        .sum()
        .unstack(fill_value=0)
    )

    root_pivot.columns = [
        f"event_root_{safe_name(c)}_count" for c in root_pivot.columns
    ]

    root_pivot = root_pivot.reset_index()

    # Country count features
    top_countries = (
        event_long
    ...
    country_df = event_long[event_long["ActionGeo_CountryCode"].isin(top_countries)].copy()

    country_pivot = (
        country_df
        .groupby(["date", "ActionGeo_CountryCode"])["n_events"]
        .sum()
        .unstack(fill_value=0)
    )

    country_pivot.columns = [
        f"event_country_{safe_name(c)}_count" for c in country_pivot.columns
    ]

    country_pivot = country_pivot.reset_index()

    # Merge event features
    event_features = daily.merge(root_pivot, on="date", how="outer")
    event_features = event_features.merge(country_pivot, on="date", how="outer")
...
def main():
    ...
    parser.add_argument(
        "--event_long_dir",
        default="/project/hrao/GDELT/features/event_long_clean_full"
    )
    parser.add_argument(
        "--output_file",
        default="/project/hrao/GDELT/modeling/model_table_clean_full.parquet"
    )
    ...
    target_cols = [c for c in model.columns if c.startswith("target_rv")]
